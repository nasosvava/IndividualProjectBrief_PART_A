...
public class Utils {
    ...
    public static String checkingStringToBeOnlyLetters() {
        Scanner scanner = new Scanner(System.in); //Εδω φτιαχνω καινουριο scanner γιατι οταν χρησιμοποιω το scanner του class δεν παιζει σωστα αυτος ειναι και ο λογος που εχω στειλει το project τοσες φορες 
        String string = " ";
    ...
    /**
     * Method checks if the localDateWeek is weekend and prevent the user from
     * putting weekend date.
     *
     * @return
     */
    public static LocalDate validationAssignment() {
        LocalDate l = null;
        boolean quit = false;

        while (!quit) {
            l = validationLocalDate();
            if (l.getDayOfWeek().equals(DayOfWeek.SATURDAY) || l.getDayOfWeek().equals(DayOfWeek.SUNDAY)) {
                System.out.println(colour.TEXT_RED + "Your date is weekend please change it." + colour.TEXT_RESET);
            } else {
                quit = true;
            }
        }
